...
function InstrumentsGallery({ specificProductsRequired, setShowCart }) {
  ...
  const handleAddToFavoritesClick = async (id) => {
    ...
    setIsFavorite(
      allFavoriteProducts.some(
        (product) => product.id == instrumentDisplayed.id
      )
    );
  };
  ...
  }, [allFavoriteProducts, instrumentDisplayed]);
          ...
          <button>
            {isFavorite ? (
              <BsFillHeartFill
                ...
                onClick={() => {
                  handleRemoveFromFavoriteClick(Number(instrumentDisplayed.id));
                }}
              ></BsFillHeartFill>
            ) : (
              <BsHeart
                ...
                className="absolute top-6 right-10 z-20 text-primary hover:text-secondary"
                onClick={() => {
                  handleAddToFavoritesClick(Number(instrumentDisplayed.id));
                }}
              ></BsHeart>
            )}
          </button>
